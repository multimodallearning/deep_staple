...
# When writing code keep the following template in mind:
### see https://github.com/MDL-UzL/UtilityFunctions/blob/main/template_function.py
###

def get_cam_overlay2d(images:torch.tensor, cams:torch.tensor):
    r"""Returns images overlayed with CAMs as matplotlib axes.
    Parameters:
        images (torch.tensor): The background images i.e. CT grayscale.
            Dimensions need to be BxHxW.
        cams (torch.tensor): The class activation maps corresponding to the
            background images.
    Returns:
        (tbd): 1d Tensor (B) containing the distance per batch sample.
    """
    # TODO: Test this. Not finished.
    _, axes = plt.subplots(nrows=1, ncols=images.size(0))
    torch.tensor()
    for idx, img in enumerate(images):
        axes[0, idx].imshow(img.detach().cpu(), cmap='gray')
        axes[0, idx].imshow(cams[idx].detach().cpu(), cmap='jet', alpha=.2, interpolation='bilinear')

    return axes



def get_anim_overlay_slices3d(images:torch.tensor, cams:torch.tensor):
    pass
...
def visualize_seg(in_type, reduce_dim=None,
    ...
    if crop_to_non_zero_seg or crop_to_non_zero_gt:
        ...
        if crop_to_non_zero_seg:
            ...
            selected_idxs = torch.cat([selected_idxs, idx_depth_with_segs])

        if crop_to_non_zero_gt:
            idx_depth_with_gt, *_ = torch.nonzero(ground_truth > 0, as_tuple=True)
            selected_idxs = torch.cat([selected_idxs, idx_depth_with_gt])
    ...
    if selected_idxs.numel() > 0:
        ...
        TILE_H = IM_H//((img_slices.shape[0]//n_per_row)+1)
