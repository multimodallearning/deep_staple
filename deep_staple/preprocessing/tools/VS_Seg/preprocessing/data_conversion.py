...
def override_warnings_at_load(widget):
    ...
    # Now override function to not display errors
    widget.warnUserIfLoadableWarningsAndProceed = types.MethodType(warnUserIfLoadableWarningsAndProceed, widget)
...
def import_T1_and_T2_data(input_folder, case_number):
    ...
    override_warnings_at_load(dicomWidget.browserWidget)
...
def cleanup_node(ndd):
    if hasattr(ndd, 'GetStorageNode'):
        slicer.mrmlScene.RemoveNode(ndd.GetStorageNode())
    # if hasattr(ndd, 'GetDisplayNode'):
    #     slicer.mrmlScene.RemoveNode(ndd.GetDisplayNode())
    slicer.mrmlScene.RemoveNode(ndd)
...
def main(argv):
    ...
    for i in range(len(patient_dirs)):
        ...
        RTSS1 = None
        RTSS2 = None
        segmentationNode_T1 = None
        segmentationNode_T2 = None
        lm_node_in_T2_list = []
        lm_node_in_T2_list = []
        transformNode = None
        ...
        for ndd in all_nodes:
            if not ndd is None: cleanup_node(ndd)
        for ndd in lm_node_in_T1_list:
            cleanup_node(ndd)
